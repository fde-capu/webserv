...
{ anounce URI_ALIAS \
\
	'Call is made to uri_alias, but is served by directory \n
	 uri_substitution, as per configuration.' \
\
; } 2> /dev/null;

cmd="curl http://$name_server:3490/uri_alias/";
testfile="$MYDIR/confs/html/uri_substitution/index.htm";
code="200";
show_output="true";
unittest "Get directory";

#################################################################

{ anounce ALIAS_GET_FILE \
\
	'Get aliases directory calling a file.' \
\
; } 2> /dev/null;

cmd="curl http://$name_server:3490/uri_alias/get-a-file";
testfile="$MYDIR/confs/html/uri_substitution/get-a-file";
code="200";
show_output="true";
unittest "Get alias file";

##################################################################

{ anounce ANOTHER_SITE \
\
	':3491, another port, another server, another directory: \n
...
testfile="$MYDIR/confs/html/index.htm";
code="200";
unittest "Redirect directory GET";

##################################################################

{ anounce REDIRECT_FILE_CALL \
\
	'Call a file when redirecting, full URI should persist.' \
\
; } 2> /dev/null;

cmd="curl -L http://$name_server:3493/monkey.gif";
testfile="$MYDIR/confs/html/monkey.gif";
code="200";
unittest "Uri must follow";

##################################################################

{ anounce REDIRECT_SUBDIR \
\
	'URI persistence on 301 calling some subdirectory.' \
\
; } 2> /dev/null;

cmd="curl -L http://$name_server:3493/somesub";
testfile="$MYDIR/confs/html/somesub/index.htm";
code="200";
unittest "Sub calls 301";

##################################################################

{ anounce REDIRECT_ALIAS_DIR \
\
	'uri_alias is aliased by 3490.' \
\
; } 2> /dev/null;

cmd="curl -L http://$name_server:3493/uri_alias";
testfile="$MYDIR/confs/html/uri_substitution/index.htm";
code="200";
unittest "3493 to 3490 to aliased";

##################################################################

{ anounce REDIRECT_ALIAS_FILE \
\
	'Getting a file on redirect + alias.' \
\
; } 2> /dev/null;
